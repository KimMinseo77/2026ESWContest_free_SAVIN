...
> **쓰레기봉투에서 멀어질수록 -20점 감점**되어 실제 위협이 아닌 단순 통과는 낮은 점수로 처리.
>까마귀의 경우는 기본점수 10점을 추가.

산출된 최종 위험 점수에 따라 시스템 동작을 4단계로 구분하며 동물 종류·주야간에 따라 서로 다른 액추에이터 조합으로 대응.
...
Orange Board에 `STOP` 명령을 전송하여 액추에이터를 안전 상태로 전환.
